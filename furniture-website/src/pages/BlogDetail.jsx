      ...
      />
          {/* <div className="space-y-6 text-theme leading-7">
            {blog.content.map((item, index) => {
              if (item.type === "heading") return <h3 key={index} className="text-xl font-semibold text-primary">{item.text}</h3>;
              if (item.type === "quote") return <blockquote key={index} className="border-l-4 border-primary pl-6 italic text-primary bg-gray-50 py-4 rounded">{item.text}</blockquote>;
          ...
          <div className="mt-12 flex gap-2 flex-wrap">
            {blog.tags.map((tag, i) => (
              <span key={i} className="px-4 py-1 text-sm border rounded-full hover:bg-primary hover:text-white cursor-pointer">{tag}</span>
            ))}
          </div> */}
        </div>
        ...
        <aside>
          ...
          <ul className="space-y-3 text-theme text-sm">
            {recentBlog.map(b => (
              <li key={b.id} className="hover:text-primary">{b.title}</li>
            ))}
          </ul>
        </aside>
      </div>
